...
/**
 * This is a Tailwind CSS port of `@atlaskit/pragmatic-drag-and-drop-react-drop-indicator/box`
 */
export function DropIndicator({ edge, gap }: { edge: Edge; gap: string }) {
  ...
  return (
    <div
      ...
      className={`absolute z-10 bg-blue-700 pointer-events-none before:content-[''] before:w-[var(--terminal-size)] before:h-[var(--terminal-size)] box-border before:absolute before:border-[var(--line-thickness)] before:border-solid before:border-blue-700 before:rounded-full ${orientationStyles[orientation]} ${edgeStyles[edge]}`}
    ></div>
  );
}
